...
def generate(Gs, noise_seed, val=1):
    ...
    # 2000フレームごとに動画を保存
    match = re.search(r'(\d+)\.pkl$', a.model)

    if match:
        number = int(match.group(1))
        print("resume idx:", number)
    else:
        number=0
        print("Number not found")

    for start_frame in range(number, frame_count, 2000):
...
if __name__ == '__main__':
    # 途中からの再開機能
    base_model = '/content/drive/MyDrive/HITSUJI2023/models/network-snapshot-000012.pkl'
    pkl_name = osp.splitext(base_model)[0]
    start_variation = 0
    ...
    for i in range(start_variation, a.variations):
        with dnnlib.util.open_url(pkl_name + '.pkl') as f:
            ...
            Gs.load_state_dict(torch.load(a.model))
        generate(Gs, a.noise_seed, val=i)
