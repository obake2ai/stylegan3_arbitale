...
def generate(noise_seed):
    ...
    if a.prune is not None:
        out_name+=f'_Multipruned'
        for name, module in Gs.named_modules():
            if 'L3_' in name: prune_module(name, module, 0.05)
            if 'L4_' in name: prune_module(name, module, 0.12)
            if 'L5_' in name: prune_module(name, module, 0.08)
            if 'L10_' in name: prune_module(name, module, 0.06)
            if 'L11_' in name: prune_module(name, module, 0.06)
            if 'L12_' in name: prune_module(name, module, 0.08)
            if 'L14_' in name: prune_module(name, module, 0.12)
